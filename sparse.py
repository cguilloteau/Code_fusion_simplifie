...
This code computes the sparse matrix A and the vector b used in the fast fusion precedure described in [2].
"""
...
def M(i, j, phv, nr, nc):
    res = np.zeros(nr*nc, dtype=np.cdouble)
    nf = phv.shape[0]
    for m in range(nf):
...
def PHV(lacp, P, V, nr, nc):
    nf = P.shape[0]
    lh = V.shape[0]
    res = np.zeros((nf, lacp, nr*nc), dtype=np.cdouble)
#    print(' *** PHV computation ***')
    for m in range(nf):
        for i in range(lacp):
            sum_h = np.zeros(nr*nc, dtype=np.cdouble)
            for l in range(lh):
...
def Anc(lacp, nr, nc, P, V):
    ...
    data = np.zeros((lacp**2, nr*nc), dtype=np.cdouble)
    for i in range(lacp):
        # print('i='+str(i))
    ...
    return sp.coo_matrix((data, (row, col)), shape=(lacp*nr*nc, lacp*nr*nc), dtype=np.cdouble), phv
...
def Anc_bis(lacp, nr, nc, P, V):
    ...
    t2 = time()
    print('Anc computation time : '+str(t2-t1)+'s.')
    return sp.coo_matrix((data, (row, col)), shape=(lacp*nr*nc, lacp*nr*nc), dtype=np.cdouble)
...
def C(i, j, V, row, col, nr, nc, d):
    res = np.zeros(nr*nc*d**2, dtype=np.cdouble)
    lh = len(V)
    for m in range(lh):
...
def Ans(lacp, nr, nc, d, V, Q):
    ...
    col = np.matlib.repmat(col, 1, lacp)[0]
    mat = np.reshape(np.reshape(np.arange(lacp**2), (lacp, lacp)).T, lacp**2)
    data = np.zeros((lacp**2, nr*nc*d**2), dtype=np.cdouble)
    for i in range(lacp):
        # print('i='+str(i))
        for j in range(lacp-i):
            # print('j='+str(j+i))
...
#    bnc=np.zeros((lacp,nr*nc))
    ...
    return np.reshape(sbnc*bnc, np.prod(bnc.shape)), np.reshape(sbns*bns, np.prod(bns.shape))
