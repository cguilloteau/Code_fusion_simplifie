...
def get_spa_psf_ms(sigma=0):
    ...
    h_ = fits.getdata(PSF+'nc_GPSF_FFT.fits')
    k, l, m, n = h_.shape
    h = h_[0]+h_[1]*1.j
...
def get_spa_bandpsf_ms(band, sigma=0):
    ...
    h_ = fits.getdata(PSF+'nc_GPSF_FFT.fits')[:, band]
    k, m, n = h_.shape
    h = h_[0]+h_[1]*1.j
...
def produce_MS_nir_bis(M, A, tabwave, channel, snr=50, sigma=0):
    ...
    # Spectral degradation operator using the fits file
    Lm = fits.getdata(DATA+'Lmlow.fits')
...
def get_spa_psf_hs(sigma=0):
    # Get the whole spatial Point Spread Function matrix. It has to be calculated in the Fourier domain and saved in advance with webbpsf.
    g_ = fits.getdata(PSF+'ns_GPSF_FFT.fits')
    k, l, m, n = g_.shape
    g = g_[0]+g_[1]*1.j
...
def get_spa_bandpsf_hs(band, sigma=0):
    # Get a spatial Point Spread Function at wavelength number 'band'. It has to be calculated in the Fourier domain and saved in advance with webbpsf.
    g_ = fits.getdata(PSF+'ns_GPSF_FFT.fits')[:, band]
    k, m, n = g_.shape
    g = g_[0]+g_[1]*1.j
...
def produce_HS_nir(M, A, tabwave, fname, dname, snr=50, sigma=0):
    ...
    Lh = fits.getdata(DATA+'Lhlow.fits')
...
def produce_HS_nir_bis(M, A, tabwave, fname, dname, snr=50, sigma=0, d=0.31):
    ...
    for i in range(lh):
        ...
        Y[i] = subsample2d(np.real(_centered(np.fft.ifft2(H*np.fft.fft2(X, norm='ortho'), norm='ortho')[:-2, :-2], (p, q))))
    # Spectral covolution and spectral throughput
    Y = np.reshape(Y, (lh, p_*q_))
    Y = np.reshape((Lh*np.apply_along_axis(lambda x: np.convolve(x, L, mode='same'), axis=0, arr=Y).T).T, (lh, p_, q_))
    return Y
...
def create_gpsf(tabwave, psf_size, pix_size, filename, npad=11):
    # Full width at half maximum in rad->arcsec->pixels
    fwhm = (3600*180/np.pi)*(1.22e-6*tabwave[:,0]/6.5)/pix_size
    sigma = fwhm*(2*np.sqrt(2*np.log(2)))**(-1)
    psf_mat = np.zeros((2,tabwave[:,0].size,psf_size+2*npad+2,psf_size+2*npad+2))
    for i in np.arange(sigma.size):
        # psf = np.zeros((psf_size+2*npad+2,psf_size+2*npad+2))
        psf = gaussian_filter(psf_size+2*npad+2, sigma=sigma[i])
        psf = psf/np.sum(psf)
        psf = np.fft.fftshift(psf)
        psf = np.fft.fft2(psf)
        psf_mat[0,i] = np.real(psf)
        psf_mat[1,i] = np.imag(psf)
    hdu = fits.PrimaryHDU(psf_mat)
    hdu.writeto('Data/PSF/'+filename,overwrite=True)


def gaussian_filter(kernel_size, sigma=1, muu=0):
    # Initializing value of x,y as grid of kernel size
    # in the range of kernel size
    x, y = np.meshgrid(range(-kernel_size//2,kernel_size//2),range(-kernel_size//2,kernel_size//2))
    dst = np.sqrt(x**2+y**2)
...
def main(file_data='Data/', file_save='Data/', snr=50, sigma=0):
    ##### Get the 3D scene with full resolution
    M = fits.getdata(file_data+'M_1_low.fits').T
    A = fits.getdata(file_data+'Alowcentered.fits')
    for i in range(4):
        Amin = np.min(A[i])
    ...
    tabwave = fits.getdata(file_data+'tabwave_low.fits')[:, 0]
    channel = 'short'
    fname = 'na'
    dname = 'na'
    #####

    ##### Compute gaussian PSFs.
    psf_size = 120
    npad = fact_pad
    pix_size = 0.031
    create_gpsf(tabwave, psf_size, pix_size, 'nc_GPSF_FFT.fits', npad=npad)
    pix_size = 0.01
    create_gpsf(tabwave, psf_size, pix_size, 'ns_GPSF_FFT.fits', npad=npad)
    #####

    ##### Compute images
    print('Simulating HS and MS images ...')
    Yh_highsnr = apply_noise(produce_HS_nir_bis(M, A, tabwave, fname, dname, snr, sigma), 'nirspec')
