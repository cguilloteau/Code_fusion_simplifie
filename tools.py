...
def compute_translation(sh):
    K, L = sh
    trans = np.zeros(sh, dtype=np.cdouble)
    for k in range(K):
        for l in range(L):
...
def get_trans(name):
    ...
    L, M, N = t.shape()
    trans = np.zeros((M, N), dtype=np.cdouble)
    trans = t[0]+t[1]*1.j
    return trans
...
def get_psf_ns(i):
    i_ = fits.getdata(PSF+'ns_GPSF_FFT.fits').shape[0]//2+i
    g = np.array(fits.getdata(PSF+'ns_GPSF_FFT.fits')[i]+fits.getdata(PSF+'ns_GPSF_FFT.fits')[i_]*1.j, dtype=np.cdouble_)
    return g


def get_psf_nc(i):
    i_ = fits.getdata(PSF+'nc_GPSF_FFT.fits').shape[0]//2+i
    h = np.array(fits.getdata(PSF+'nc_GPSF_FFT.fits')[i]+fits.getdata(PSF+'nc_GPSF_FFT.fits')[i_]*1.j, dtype=np.cdouble_)
    return h


def get_h(mode='direct'):
    # h_ = fits.getdata('H_fft_1.fits')
    h_ = fits.getdata(PSF+'ns_GPSF_FFT.fits')
    k, l, m, n = h_.shape
    h = np.zeros((l, m, n), dtype=np.cdouble)
    h = h_[0]+h_[1]*1.j
    if 'adj' in mode:
        h = np.conj(h)
...
def get_h_mean(mode='direct'):
    ...
    h_ = fits.getdata(PSF+'nc_GPSF_FFT.fits')[:, :, 0, 0]
...
def get_h_band(band, mode='direct'):
    ...
    k, m, n = h_.shape
    h = np.zeros((m, n), dtype=np.cdouble)
    h = h_[0]+h_[1]*1.j
    if 'adj' in mode:
        h = np.conj(h)
...
def get_h_bands(lh, l_1000, mode='direct'):
    if l_1000 == (lh//1000)+1:
        h_ = fits.getdata(PSF+'nc_GPSF_FFT.fits')[:, l_1000*1000:]
    else:
        h_ = fits.getdata(PSF+'nc_GPSF_FFT.fits')[:, l_1000*1000:(l_1000+1)*1000]
    k, l, m, n = h_.shape
    h = np.zeros((l, m, n), dtype=np.cdouble)
    h = h_[0]+h_[1]*1.j
    if 'adj' in mode:
...
def get_g(mode='direct'):
    ...
    k, l, m, n = g_.shape
    g = np.zeros((l, m, n), dtype=np.cdouble)
    g = g_[0]+g_[1]*1.j
    if 'adj' in mode:
        g = np.conj(g)
...
def get_g_mean(mode='direct'):
    ...
    g_ = fits.getdata(PSF+'ns_GPSF_FFT.fits')[:,:,0,0]
    # g_ = fits.getdata(DATA+'G_fft.fits')[:,:,0,0]
    k, l = g_.shape
    g = np.zeros((l), dtype=np.cdouble)
    g = g_[0]+g_[1]*1.j
    if 'adj' in mode:
        g = np.conj(g)
    return g
...
def get_g_band(band, mode='direct'):
    ...
    k, m, n = g_.shape
    g = np.zeros((m, n), dtype=np.cdouble)
    g = g_[0]+g_[1]*1.j
    if 'adj' in mode:
        g = np.conj(g)
...
def aliasing(Z, shape3d):
    ...
    Z_ = np.zeros((l, m//d, n//d), dtype=np.cdouble)
    for i in range(d):
        for j in range(d):
...
def aliasing_adj(Z, shape3d):
    Z_ = np.zeros(shape3d, dtype=np.cdouble)
    M = shape3d[1]//d
    N = shape3d[2]//d
